...
class CompanyStatementHistory:
    ...
    def populate_timeseries(self,statement,statement_dict,list_statement_dates,logging):
        ...
        self.check_dataframe(timeseries_df,logging)
    ...
    def populate_timeseries_df(self,statement_dict,list_statement_dates,logging):
        ...
        timeseries_df = self.adjust_for_tag_changes_and_10k(list_10k,statement_dict,timeseries_df,logging)
    ...
    def adjust_for_tag_changes_and_10k(self,list_10k,statement_dict,timeseries_df,logging): 
    ...
    def clean_up_timeseries_df(self,statement_dict,list_statement_dates,timeseries_df,logging):
    ...
    def count_sequential_quarters(self,quarter_list,log_missing=False,logging=None):
    ...
    def check_dataframe(self,timeseries_df,logging):
    ...
    def save_file(self,statement,timeseries_df,logging):

        statement_folder,statement_name = self.statement_file_path_and_name(statement,logging)
...
def add_labels_to_timeseries(self,self.timeseries_df,self.statement_dict,self.list_statement_dates):

        for ds in self.list_statement_dates:

            label_list = [l for l in self.statement_dict[ds].columns if 'label' in l .lower()]

            for label in label_list:
                if label not in self.timeseries_df.columns:
                    self.timeseries_df[label] = None

            if ds ==self.list_statement_dates[0]:
                self.timeseries_df.loc[ self.statement_dict[ds].index,label_list] = self.statement_dict[ds].loc[:,label_list]
            else:
                labels_to_find_index = self.statement_dict[ds][self.statement_dict[ds].index.isin(labels_to_find_list)].index
                self.timeseries_df.loc[labels_to_find_index,label_list] = self.statement_dict[ds].loc[labels_to_find_index,label_list] 

            label_list = [l for l in self.timeseries_df.columns if 'label' in l .lower()] 

            if self.timeseries_df[label_list].notna().any(axis=1).all() == False: #if any labels are still NA
                labels_to_find_list = self.timeseries_df[self.timeseries_df[label_list].notna().any(axis=1) == False].index
            else:
                break
        ...
        for label in label_list:
            self.timeseries_df.loc[self.timeseries_df['label'].isna(),'label'] = self.timeseries_df.loc[self.timeseries_df['label'].isna(),label]  

        self.timeseries_df['xbrl_tag'] = self.timeseries_df.index 
        self.timeseries_df = self.timeseries_df.set_index(['label','xbrl_tag']) 
        
        for label in [l for l in self.timeseries_df.columns if 'label' in l .lower()]: 
            self.timeseries_df = self.timeseries_df.drop(label,axis=1)
